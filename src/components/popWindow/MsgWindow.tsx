...
class MessagePop extends BaseClass{
    ...
    getPhoneCode(){
        ...
        new window.actions.ApiSendMobileVCodeAction().fly((resp:any)=>{
            ...
            if(resp.StatusCode === 0){
                ...
                timer=setInterval(()=>{
                    if(timerNum>0){
                        _this.setState({sendButtonMes:'重新获取('+ timerNum-- +'s)'});
                    }else{
                        _this.setState({sendButtonMes:'获取验证码',buttonDisabled:false});
                        timerNum=60;
                        clearInterval(timer);
                    }
                },1000)
    ...
    render(){
            ...
            return(
                <div className="PopWindow" onClick={()=>this.props.hidePop()}>
                    <div className="msgWindow windowBox" onClick={(event) => { event.stopPropagation() }}>
                        ...
                        <p className="windowTitle titleImg">手机短信验证</p>
                        <div className="bottomBox">
                            <p className='tips'>点击将向您的手机号发送验证码</p>
                            ...
                            <div className='inputWrap clearfix'>
                                ...
                                <a className={this.state.buttonDisabled ? "getCode disabled" : "getCode"} onClick={this.getPhoneCode.bind(this)}>
        ...
        let {title,cancleFunc,okText,okFunc,changeInputVal,inputType,isBirthday,inputVal} = this.props;
        // 个人资料修改弹窗
        return(
            <div className="PopWindow" onClick={cancleFunc}>
                <div className="infoWindow windowBox" onClick={(event) => {event.stopPropagation()}}>
                    <i className="icon iconfont icon-iconclose" onClick={cancleFunc}></i>
                        ...
                        <div className='inputWrap clearfix'>
                            {
                                ...
                                :
                                <input className="infoInput" onChange={changeInputVal} type={inputType} placeholder={"请输入您需要修改的"+title}/>  
                            }                      
                        </div>                        
                        <button className="leftBtoom" onClick={okFunc}>{okText}</button>
                    </div>
                </div>
            </div>
        )
