...
export class PageNavAction extends ApiAction {
    constructor(Tag = "", Type = "", PageNo = 1) {
        ...
        /*
            OwinId:业主名， BT6,BBT,ASA
            Tag:要嘛传皮名，返回所有导航 bet365 ， 要嘛传导航名,BBT_BET365_NAV
        */
        Tag = Tag ? Tag : window.config.spec.substr(0, window.config.spec.indexOf("-"));
        //let ownerName = config.spec.substr(config.spec.indexOf("-")+1);
...
export class ApiTransferAction extends ApiAction {
    constructor(GamePlatform:string,type:string,Amount:string|number,IsDemo:boolean = false,OperateWebSite = WebSiteOperate) {
